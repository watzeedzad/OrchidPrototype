...
export default class AddFarm {
    ...
    async operation(req, res) {
        ...
        await addFarm(ownerName, ownerSurname, ownerTel, ownerAddress, configFilePath, pimac, function (addFarmResult) {
            if (addFarmResult) {
                res.sendStatus(200);
            } else {
                res.sendStatus(500)
...
async function addFarm(farmId, ownerName, ownerSurname, ownerTel, ownerAddress, configFilePath, pimac, callback) {
    ...
    await farmData.save(function (err, dox) {
        if (err) {
            addFarmResult = false;
